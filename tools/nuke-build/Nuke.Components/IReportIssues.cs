...
{
    [PublicAPI]
    public interface IReportIssues : IRestore, IHaveReports
    {
        ...
        IEnumerable<(string PackageId, string Version)> InspectCodePlugins => Array.Empty<(string PackageId, string Version)>();
        ...
        IEnumerable<string> InspectCodeFailOnIssues => Array.Empty<string>();
        IEnumerable<string> InspectCodeFailOnCategories => Array.Empty<string>();
        ...
        void InspectCodeCheckResults()
        {
            ...
            foreach (var (TypeId, Message, File, Line, Severity, CategoryId) in issues)
            {
                if (Severity == nameof(ReSharperSeverity.WARNING) &&
                    !InspectCodeReportWarnings &&
                    !InspectCodeFailOnIssues.Contains(TypeId) &&
                    !InspectCodeFailOnCategories.Contains(CategoryId))
                    continue;

                var message = InspectCodeIssueMessage(TypeId, File, Line, Message);
